...
def edit_torrent(torrent_id):
    ...
    if flask.request.method == 'POST' and form.submit.data and form.validate():
    ...
    elif flask.request.method == 'POST' and delete_form.validate() and ban_form.validate():
        ...
        if editor.is_moderator:
            tbanned = models.Ban.banned(None, torrent.uploader_ip)
            print(tbanned)
            tbanned = tbanned.first()
            ubanned = True
            if torrent.user:
...
def _delete_torrent(torrent, form, banform):
    ...
    ban_torrent = (form.ban.data or banform.ban_user.data or banform.ban_userip.data)
    ...
    elif banform.ban_user and torrent.uploader_id:
        action = 'banned'
        torrent.banned = True
        if not torrent.deleted:
            torrent.deleted = True
    ...
    if not editor.is_moderator or not (banform.ban_user.data or banform.ban_userip.data):
        return flask.redirect(url)
    ...
    reason = "[{0}#{1}]({2}) {3}".format(flavor, torrent.id, eurl, form.reason.data)
    ...
    flask.flash(flask.Markup('Uploader has been successfully banned.'.format(action)), 'success')
